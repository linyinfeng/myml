...
dangerousVariable (TyMu x t) = let dt = dangerousVariable t in
  if x `Set.member` dt then freeVariable t -- x included in dangerous variables
  else dt
...
dangerousVariableRecordRow (TyRow f cof) = Map.foldl (\a b -> a `Set.union` dangerousVariableRecordPresence b) Set.empty f `Set.union` freeVariable cof
