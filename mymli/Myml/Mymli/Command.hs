...
processCommand (CmdLoadFile file) = do
  ...
  case result of
    ...
    Just inputs -> do
      ...
      when success
        (mymliMergeFileEnv fileEnv >> mymliGc)
      return MymliContinue
...
mymliEnvForFile = do
  ...
  return MymliEnv {
    envStore = Store Map.empty (storeMinFree envStore),
...
mymliMergeFileEnv fileEnv = do
    let MymliEnv {
      envStore = Store fileStoreMap fileStoreMinFree,
      envTermBindings = fileTermBindings,
      envValueBindings = fileValueBindings,
      envTypeBindings = fileTypeBindings,
      envInferState = fileInferState
    } = fileEnv
    MymliEnv {
      envStore = Store currentStoreMap _currentStoreMinFree,
      envTermBindings = currentTermBindings,
      envValueBindings = currentValueBindings,
      envTypeBindings = currentTypeBindings,
      envInferState = _currentInferState
    } <- get
    let env = MymliEnv {
      envStore = Store {
        storeData = Map.unionWith (\_ _ -> error "store collied after file loading") currentStoreMap fileStoreMap,
        storeMinFree = fileStoreMinFree
      },
      envTermBindings = Map.unionWith const fileTermBindings currentTermBindings,
      envValueBindings = Map.unionWith const fileValueBindings currentValueBindings,
      envTypeBindings = Map.unionWith const fileTypeBindings currentTypeBindings,
      envInferState = fileInferState
    }
    put env
