...
regTreeEqTests = testGroup
  ...
  , testCase "Empty row"
    ...
    $            pType "\x3bc List . [ `cons : Present { head : Present X, tail : Present List }, `nil : Present Unit ]"
    `regTreeEq'` pType "[ `cons : Present { head : Present X, tail : Present \x3bc List . [ `cons : Present { head : Present X, tail : Present List }, `nil : Present Unit ] }, `nil : Present Unit ]"
    @?=          True
  ]
